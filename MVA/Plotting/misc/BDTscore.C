...
void BDTscore(){
	...
	string 	version 			= "v10";
	string 	category 			= "boosted"; // vbf boosted 1jet 0jet
	string	region 				= "SR"; // SR  CR
	string 	tag			       	= "50Trees_500LeafEvts_NoHpT_QCDnotOS"; // Katy, sorry for changing this :)
	string	purpose 			= "Plot";
	int 	Hmass 				= 125;
	...
	///blind - unblind
	bool kShowData = region == "SR"; /// SR is blind for the moment, no data overlaid - use SR for limits
	...
	if(category == "vbf"){
		...
		//		QCD_TrackFitFactor = 0.434535;Z_TrackFitFactor = 0.0737466;//MedMed CR
		//		QCD_TrackFitFactor = 0.546003;  Z_TrackFitFactor =0;//Reverse taudEta CR
...
// 	gPad->Update();
	...
	string thisregion = region == "CR" ? "Control Region" : region == "SR" ? "Signal Region" : "Which Region?";
