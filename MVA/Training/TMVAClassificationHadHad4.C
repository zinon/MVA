...
// @(#)root/tmva $Id: TMVAClassificationHadHad4.C 105751 2013-07-22 16:52:23Z zenon $
/**********************************************************************************
*
...
void TMVAClassificationHadHad4( TString myMethodList = "" )
{
  ///to define:
  string	PartUsedForTraining = "B"; // A, B, AB
  string 	treename			= "mva";
  string 	purpose				= "MVA";
  string 	category 			= "vbf"; // physics-based categories: vbf -  boosted - 1jet - 0jet
  string	region 				= "CR_NoMassCut";//"all"; //all now means MMC > min MMC cut
  string 	version 			= "v16";
  bool kEmbedding = true; // false with default to alpgen
...
}else if (category == "boosted" || category == "1jet" || category == "0jet"){  ///only ggF signal (and VH ?)
m_SignalTestTrain.push_back("VBF:100:0:0");		m_SignalTestTrain.push_back("GGF:100:0:0");
m_SignalTestTrain.push_back("VBF:105:0:0");		m_SignalTestTrain.push_back("GGF:105:0:0");
m_SignalTestTrain.push_back("VBF:110:0:0");		m_SignalTestTrain.push_back("GGF:110:0:0");
m_SignalTestTrain.push_back("VBF:115:0:0");		m_SignalTestTrain.push_back("GGF:115:0:0");
m_SignalTestTrain.push_back("VBF:120:0:0");		m_SignalTestTrain.push_back("GGF:120:0:0");
m_SignalTestTrain.push_back("VBF:125:1:1");		m_SignalTestTrain.push_back("GGF:125:1:1");
m_SignalTestTrain.push_back("VBF:130:0:0");		m_SignalTestTrain.push_back("GGF:130:0:0");
m_SignalTestTrain.push_back("VBF:135:0:0");		m_SignalTestTrain.push_back("GGF:135:0:0");
m_SignalTestTrain.push_back("VBF:140:0:0");		m_SignalTestTrain.push_back("GGF:140:0:0");
m_SignalTestTrain.push_back("VBF:145:0:0");		m_SignalTestTrain.push_back("GGF:145:0:0");
  ...
  //        if (ApplyEventWeights){
	  ...
	  for(ItrStr it = m_SignalTestTrain.begin(); it != m_SignalTestTrain.end(); it++){
		...
		if(kTrain){
		  ...
		  factory->AddTree(tmap[mcid], 
						   "Signal", mcb.Get_Norm(mcid),  cutSignalTrain, "train");
		}
		if(kTest){
		  cout<<"Adding "<<proc<<" mH = "<<mass<<" GeV in Testing"<<endl;
		  factory->AddTree(tmap[mcid], 
						   "Signal", mcb.Get_Norm(mcid) , cutSignalTest,  "test");
		}
	  }
	  ...
	  if(kAddQCD){
		...
		factory->AddTree (tdata, "Background", 1.0, cutBackgroundTrain, "train"); 
		factory->AddTree (tdata, "Background", 1.0, cutBackgroundTest, "test");
	  }
	  
	  ///EMBEDDING. 
	  if(kEmbedding){	  
		factory->AddTree (tEmbedding, "Background", 1.0, cutBackgroundTrain, "train"); 
		factory->AddTree (tEmbedding, "Background", 1.0, cutBackgroundTest, "test");
	  }
	  ///End Embedding
