...
bool Loop(const string &myCategory_s, const string &myVariable_s){
	...
	if( myVariable_s == "met_et"){ 
		...
		if(myCategory_s=="1jet" || myCategory_s== "boosted"){  MinX = 0.0; MaxX = 3.4; MinY = 1e-6; MaxY = 800.0; }
		...
		if(myCategory_s=="1jet"){ MinX = 60; MaxX = 260; MinY = 1e-6; MaxY = 1000.0; }
	...
	///plot making
	cout<<"\nCuts for search region : \n"<<hc.GetCut(category, "OS", purpose,region).GetTitle()<<endl;
...
// 	gPad->Update();
	...
	//	string thisregion = region == "CR" ? "Control Region" : region == "SR" ? "Signal Region" :region == "all" ? "No Mass Window" : "which Region??";
	string thisregion = region == "CR" ? "Control Region" : region == "SR" ? "Signal Region" :region == "all" ? "Reverse #tau#tau #Delta#eta CR" : "which Region??";
